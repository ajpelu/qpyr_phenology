...
elong <- raw_elong %>% 
  ...
  filter(ELONGACION == 'Elong 1') %>% 
  select(loc = LOCALIDAD, 
        id_ind = COD_EJEMPLAR,
        rama = RAMA, 
        elongacion = LONG_ELONG, 
        cota, replica, date, year)
  

# Summary n 
n <- elong %>% 
  group_by(loc, cota, year) %>% 
  summarise(mean=mean(elongacion)) 

ggplot(n, aes(x=year, y=mean)) + geom_bar(stat='identity', position='dodge') + facet_grid(loc~cota, scales = 'free')
